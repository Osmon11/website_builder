...
export default function (
  ...
  const computedArray = $options.computed
  ...
  traverse(ast, [
    (
      ...
      if (templateAst.type === ELEMENT) {
        ...
        templateAst.type === INTERPOLATION
      ) {
        try {
    ...
    Identifier(identifier: any, ancestors: any) {
      ...
      if (
        ...
        ancestors[ancestors.length - 2].type ===
          "CallExpression" &&
        ancestors[ancestors.length - 2].callee
          .name === varName
      ) {
        return;
      } else if (
...
export function traverse(
  ...
  const availableVarNamesThisLevel =
